...
class Simulation:
    ...
    def simulate_departure(self):
        ...
        while self.aircraft.current_alt < self.airspace.alt_bands.alt_band_mid.min_alt:
            ...
            self.aircraft.update_altitude(rate_of_climb_or_descent=3000 / 60, time_elapsed=1)  # 3000 ft/min, converted to ft/second

            # Output the current state
            print(f"Aircraft {self.aircraft.aircraft_type} departing, current altitude: {self.aircraft.current_alt:.2f} feet, position: ({self.aircraft.current_lat:.4f}, {self.aircraft.current_long:.4f})")
    ...
    def simulate_arrival(self):
        ...
        while self.aircraft.current_alt > 0:
            ...
            print(f"Aircraft {self.aircraft.aircraft_type} arriving, current altitude: {self.aircraft.current_alt:.2f} feet, position: ({self.aircraft.current_lat:.4f}, {self.aircraft.current_long:.4f})")
